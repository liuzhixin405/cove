...
- **REPL 模式**（fallback）：行式 ANSI 转义序列，管道/重定向时使用
...
│   ├── repl_loop.go                #   REPL 交互循环（fallback模式）(347行)
...
main()                                     // cli/cove/main.go:75
  ...
  │     │
  │     └─ false → runREPL()               // repl_loop.go（管道/--no-tui）
  │                 │
  │                 ├─ 无限循环：显示提示符 → 读用户输入
...
| **repl** | `repl/` | ANSI 颜色 + 自定义 readline |
...
在 `cli/cove/repl_loop.go` 中已经存在的命令路由器会自动处理（因为命令注册表已更新）。
