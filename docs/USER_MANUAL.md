...
| `provider.name` | string | 提供商名称 |
| `provider.api_key` | string | API 密钥 |
| `provider.base_url` | string | 自定义 API 端点 |
| `permission_mode` | string | 默认权限模式 |
| `max_budget_usd` | number | 会话预算上限（美元） |
| `thinking_tokens` | number | thinking/推理 token 预算 |
| `debug` | boolean | 调试模式 |
...
连续 6+ 次工具调用失败在 30 秒内时触发断路，阻止失控的工具循环。成功后重置窗口。
