...
func InstallSkill(name, source, url string) error {
	...
	dir := filepath.Join(home, ".agentgo", "skills", name)
...
var bundles = []Skill{
	...
	{Name: "update-config", Description: "Read and modify agentgo configuration files.", Prompt: "CONFIG UPDATE WORKFLOW:\n1. Read the existing config file (~/.agentgo/config.json)\n2. Identify the setting to change\n3. Make the change precisely\n4. Validate the JSON is still valid\n5. Inform the user of the change (restart may be required)"},
...
func LoadAll(m *Manager, cwd string) {
	...
	home, _ := os.UserHomeDir()
	if home != "" {
		m.AddDirectory(filepath.Join(home, ".agentgo", "skills"))
		m.AddDirectory(filepath.Join(home, ".claude", "skills"))
	}

	if cwd != "" {
		m.AddDirectory(filepath.Join(cwd, ".claude", "skills"))
		m.AddDirectory(filepath.Join(cwd, ".agentgo", "skills"))
