...
func (c *MemoryCmd) Help() string        { return "/memory [list|add|remove] - 管理持久记忆文件" }
func (c *MemoryCmd) Execute(ctx context.Context, in Input) (Output, error) {
	...
	default:
		return Output{Message: "用法: /memory [list|add|remove]"}, nil
	}
}
...
func (c *DreamCmd) Execute(ctx context.Context, in Input) (Output, error) {
	...
	if !cfg.Enabled {
		return Output{Message: "自动梦境已禁用。请在 ~/.agentgo/dream.json 中启用。"}, nil
	}
