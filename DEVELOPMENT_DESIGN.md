...
> 版本: 1.0 | 基于 Cove v6.3.1 | 参考: Gemini CLI (google-gemini/gemini-cli)
...
1. **理解业务，而非翻译代码** — Gemini CLI (TypeScript/Node.js) 作为**业务参考**，学习它解决了什么问题、为什么这样设计。Cove (Go) 的实现用自己的架构、自己的惯用写法、自己的取舍。不做代码翻译机。
2. **最小侵入** — 新代码通过接口注入 Engine，不破坏现有逻辑
3. **优雅降级** — 每个子系统可选，不可用时不影响核心对话功能
4. **100% 完整实现** — 每个模块必须完整可用：完整的数据结构、完整的错误处理、完整的测试覆盖、完整的 TUI 用户感知。不允许 demo / 骨架 / TODO。
5. **Go 惯用写法** — 遵循现有代码风格（短变量名、显式错误处理、goroutine + context、接口抽象）
6. **向后兼容** — 现有配置文件和命令行参数不受影响，新功能默认启用但可关闭

**“参考而非翻译”示例**：

| Gemini CLI 做了什么 | 我们怎么理解 | Cove 怎么做 |
|---------------------|-------------|------------|
| `loopDetectionService.ts` 用 `createHash('sha256')` 对 toolCall 做哈希 | 核心思路：对「工具名+关键参数」做指纹，重复达到阈值就中断 | 用 Go 的 `crypto/sha256`，但参数提取逻辑按 Cove 的工具体系自己设计——bash 看 command、read 看 filePath+offset、edit 看 filePath+oldString |
| `modelRouterService.ts` 用 `CompositeStrategy` 链式路由 | 核心思路：多个策略按优先级依次决策，前面的可以短路后面的 | 不用策略模式继承链，用 Go 的 `[]Router` 切片 + for 循环，更简洁直接 |
| `toolOutputMaskingService.ts` 用 TS 的异步迭代 | 核心思路：从后往前扫描，保护窗口+批量触发 | Go 用切片反向遍历，遮蔽后的内容用 `os.WriteFile` 落到磁盘，消息里替换为路径引用 |
| 大量 `private readonly` 字段 + getter/setter | 这是 TS/Java 的范式 | Go 用小写字段 + 包级访问控制，方法直接访问字段 |
...
Gemini CLI 的 `loopDetectionService.ts` 用三层检测解决此问题。我们采用其前两层（零额外 API 开销），第三层（LLM 裁判）作为可选增强。
...
**设计**：`internal/safety/checker.go`。注册检查器 → 输入/输出通过检查器链 → 拦截注入、密钥泄露、危险命令。

### 10.2 Next Speaker 检测

**目标**：判断 AI 是否应该继续响应还是等待用户（减少无意义连续工具调用）。

**设计**：在 Engine 中工具循环结束后，调用一次简单判断（检查最近几条消息模式），如果 AI 已经在"等待用户输入"，则提前结束循环。

### 10.3 遥测系统

**目标**：匿名使用数据收集。

**设计**：`internal/telemetry/telemetry.go`。事件队列 → 本地聚合 → 可选上报。

### 10.4 IDE 伴生

**目标**：VS Code 扩展。

**设计**：独立仓库。通过 MCP 连接 Cove Agent 进程。

### 10.5 Repomap 增强

**目标**：从当前的文件树分析升级为类关系图、调用链分析。

**设计**：增强 `internal/repomap/repomap.go`。

### 10.6 Session Diff 对比

**目标**：可视化文件变更。

**设计**：在 TUI Git 面板基础上，显示 session 内所有文件变更的 diff。
