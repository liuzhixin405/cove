...
// the first call afterwards, whatever tool or arguments it used.
func TestResetClearsRapidFailureWindow(t *testing.T) {
	...
	if got := tr.BeforeCall("read", map[string]any{"filePath": "a.go"}); got.Action != Block {
		t.Fatalf("precondition: expected the breaker to be tripped, got %v", got.Action)
	}

	tr.Reset()
	if got := tr.BeforeCall("read", map[string]any{"filePath": "a.go"}); got.Action != Allow {
