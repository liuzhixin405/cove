# Session Notes - CovePhone v4.0.5 Release

## Technical Support

For technical support regarding CovePhone, please contact:
- **Email**: 164910441@qq.com
- **GitHub Issues**: https://github.com/liuzhixin405/cove/issues

## Release Summary

- **Version**: v4.0.5
- **New**: CovePhone Android app with native Go AI engine
- **APK**: dist/v4.0.5/covephone-v4.0.5.apk
- **Mobile Engine**: mobile/cove.go
- **Android Project**: CovePhone/ (in repository root)