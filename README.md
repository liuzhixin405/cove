...
| `/tasks` | View running/queued tasks in TUI; headless reports sync execution state |
| `/stop` / `/cancel` | Cancel current task in TUI; no background queue in headless |
| `/commit [msg]` | Git add + commit |
| `/review` | Review working changes |
| `/diff` | Show git diff |
| `/doctor` | Environment quick check (Go/git/ripgrep) |
...
| `/init` | Detect project structure and initialize CLAUDE.md |
